...
public class TextFeedbackTest extends TestCase {
	...
	class TestResultPrinter extends ResultPrinter {
		...
		/* Spoof printing time so the junit.tests are deterministic
		 */
	...
	public void testEmptySuite() {
		String expected= expected(new String[]{"", "Time: 0", "", "OK (0 junit.tests)", ""});
	...
	public void testTwoTests() {
		String expected= expected(new String[]{"..", "Time: 0", "", "OK (2 junit.tests)", ""});
		TestSuite suite = new TestSuite();
		suite.addTest(new TestCase() { public void runTest() {}});
		suite.addTest(new TestCase() { public void runTest() {}});
		runner.doRun(suite);
